<!DOCTYPE html>
<html>
<head>
	<title>Archives Page</title>

	<!-- Bootstrap Core CSS -->
    <link href="assets/css/bootstrap.min.css" rel="stylesheet">

    <!-- Theme CSS -->
    <link href="assets/css/freelancer.min.css" rel="stylesheet">

    <!-- Custom Fonts -->
    <link href="assets/css/font-awesome.min.css" rel="stylesheet" type="text/css">
    <link href="assets/css/font-dl-1.css" rel="stylesheet" type="text/css">
    <link href="assets/css/font-dl-2.css" rel="stylesheet" type="text/css">
    <link href="assets/css/custom.css"  rel="stylesheet" type="text/css" >

</head>
<style>
	@media (max-width: 980px) {
		.nav-archives {
			margin-top: 130px;
		}
	}

</style>
<body>	
	<!-- Navigation -->
	<?php echo $nav ?>

	<div class="tabbable boxed parentTabs" style="margin-top: 95px;">
	    <ul class="nav nav-pills nav-archives">
	        <li role="presentation" class="active col-md-2" style="text-align: center;"><a href="#set1">Members</a>
	        </li>
	        <li role="presentation" class="col-md-2" style="text-align: center;"><a href="#set2">Magazines</a>
	        </li>
	        <li role="presentation" class="col-md-2" style="text-align: center;"><a href="#set3">Reports</a>
	        </li>
	    </ul>
	    <div class="tab-content">
	        <div class="tab-pane fade active in" id="set1">
	            <div class="tabbable">
	                <ul class="nav nav-tabs">
	                    <li class="active"><a href="#sub11">2016</a>
	                    </li>
	                    <li><a href="#sub12">2015</a>
	                    </li>
	                    <li><a href="#sub12">2014</a>
	                    </li>
	                    
	                </ul>
	                <div class="tab-content" style="margin-left: 10px;">
	                    <div class="tab-pane fade active in" id="sub11">
	                        <p>Members of 2016 of various Workplaces</p>

	                        <div class="row">
	                        <div style="padding-left: 20px; font-size: 17px">
	                        <img src="<?php echo ASSETS_URL ?>/img/team2016-17.jpg" alt="">
	                        	<div class="col-md-4">
	                        	<h4>Sector - 2</h4>
									Sankalp Kumar <br>
									Sitesh Pattanaik <br>
									Dibyajyoti Mishra <br>
									Subhankar Dutta <br>
									Nimisha Panigrahi <br>
									Rahul Kumar <br>
									Umesh Oram <br>
									Aman Verma <br>
									Chinmaya Behera <br> 
									Piyush Mathur <br>
									Sudha Priyadarshini <br>
									Mohit Chaudhary <br>
									Ayush Bagai <br>
									Sourabh Yadav <br>
								<br>
		                        <h4>Sector - 5</h4>
			                        Abhipsa Nayak <br>
									Mandava Sravya <br>
									Ankita Priyadarshini <br>
									Dhanush d pai <br>
									Surya Rohith Kotla <br>
									SK MD FAZAL ELAHI <br>
									Lokesh Sannareddy <br>
									Barnali Priyadarshini <br>
									Boddepalli Ramya <br>
									Alapati Hemalatha <br>
									Prity Hansda <br>
									Rupesh Kumar<br>
		                        <br>
		                        </div>
		                        <div class="col-md-4">
		                        <h4>Sector - 6</h4>
			                        Ayu Naveen Jain  <br>
									Samir Kumar  Nayak <br>
									Ipsita Padhy <br>
									K Pushpendu Prashant <br>
									ASHISH KUMAR BEHERA <br>
									Subham Saroj Tripathy <br>
									Sushree Abhipsa Nayak <br>
									Priyanka Prusty <br>
									Shibani Biswal  <br>
									Siddharth Mcwann  <br>
									SIDDHARTH SAHU  <br>
									Sandhyaranee Sahu  <br>
									P Santosh Kumar Patra <br>
									Subrat Sahoo <br>
									Arpit Yadav <br>
									Subhashree Sethi<br>
		                        <br>
		                        <h4>Sector - 21</h4>
			                        Binaya Kishore Behera <br>
									Teeviraj Janki <br>
									Sabyasachi Parida <br>
									Subarna Mohanty <br>
									Chinmay Mohanty <br>
									Deepa Kumari <br>
									Nitya Kumari <br>
									Deepak Kumar <br>
									Priyoda Kumar <br>
									Kartikey Dharendra <br>
									Ritesh Kumar <br>
									sandarbh kumar <br>
									Sagar chand soren<br>
		                        <br>
		                        </div>
		                        <div class="col-md-4">
		                        <h4>OSAP Colony</h4>
		                            Abhisek Khadanga <br>
									Smruti Ranjan <br>
									Chinmayee Mahapatra <br>
									Subhashree Subhasmita Mohanty <br>
									Ajaya Kumar Sahoo <br>
									Arunima Saha <br>
									Ashapriya Mohapatra <br>
									Shubhasmita Sahoo <br>
									Thimirishetty Swetha <br>
									Digant Nayak <br>
									Pratik Sejpal <br>
									Sidharth Patra <br>
									Ansuman Sahoo <br>
									Sainandan Mohanty <br>
									Ipsita <br>
									Tulasi <br>
									Diptiranjan Bindhani <br>
									Priyabrata Murmu <br>
									Debasis Acharya <br>
									Sandeep Barad <br>
									Rudra Narayan Sahoo<br>

		                        <br>
		                        <h4>Khalsa</h4>
			                        Bhanu Prakash Reddy <br>
									Shyam raj <br>
									Shubhasree mondal <br>
									Abhishek kumar <br>
									Prabhanshu Chaturvedi <br>
									Meher Sashank <br>
									Shubhashini Sahu<br>
	                        </div>
	                        </div>
	                        </div>
	                    </div>
	                    <div class="tab-pane fade" id="sub12">
	                        <p>Tab 1.2</p>
	                    </div>
	                </div>
	            </div>
	        </div>
	        <div class="tab-pane fade" id="set2">
	            <div class="tabbable">
	                <ul class="nav nav-tabs">
	                    <li class="active"><a href="#sub21">2016</a>
	                    </li>
	                    <li><a href="#sub22">2015</a>
	                    </li>
	                    <li><a href="#sub22">2014</a>
	                    </li>
	                </ul>
	                <div class="tab-content">
	                    <div class="tab-pane fade active in" id="sub21">
	                        <p>Tab 2.1</p>
	                    </div>
	                    <div class="tab-pane fade" id="sub22">
	                        <p>Tab 2.2</p>
	                    </div>
	                </div>
	            </div>
	        </div>
	        <div class="tab-pane fade" id="set3">
	            <div class="tabbable">
	                <ul class="nav nav-tabs">
	                    <li class="active"><a href="#sub31">2016</a>
	                    </li>
	                    <li><a href="#sub32">2015</a>
	                    </li>
	                    <li><a href="#sub32">2014</a>
	                    </li>
	                </ul>
	                <div class="tab-content">
	                    <div class="tab-pane fade active in" id="sub31">
	                        <p>Tab 3.1</p>
	                    </div>
	                    <div class="tab-pane fade" id="sub32">
	                        <p>Tab 3.2</p>
	                    </div>
	                </div>
	            </div>
	        </div>
	    </div>
	</div>

	<!-- Footer -->
	<div style="position: relative;bottom: 0;width: 100%; height: 100%">
	<?php echo $footer ; ?>
	</div>
<script type="text/javascript" src="assets/js/jquery.min.js"></script>
<script type="text/javascript" src="assets/js/bootstrap.min.js"></script>
<script type="text/javascript" src="assets/js/script.js"></script>
</body>
</html>